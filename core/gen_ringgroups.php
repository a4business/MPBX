	  ...
	  //echo "[ring-groups]\n";
		  $res = mysql_query("SELECT *,tenants.id as tenant_id,rg.id as rg_id,rg.name as rg_name,
		                      ...
		                      FROM t_ringgroups rg
		                           LEFT JOIN t_queues on t_queues.name = default_action_data AND default_action = 'queue' AND  t_queues.tenant_id = rg.tenant_id,
		                           tenants
		    ...
		    while($row = mysql_fetch_assoc($res)){
              ...
              $_tenant_id = $row['tenant_id'];
		    	  $_rgname = $row['rg_name'];
		    	  $_rgid = $row['rg_id'];
		    	  $_announce = basename($row['announcement_file'], '.' . end(explode('.', $row['announcement_file'])));
		          ...
		          $ring_res = mysql_query("SELECT * FROM t_ringgroup_lists rgl
		           ...
		           if (!$ring_res) die(mysql_error()."\n");
		            while($grp = mysql_fetch_assoc($ring_res)){
		            	$DATA = $grp['extensions'];
		            	$items[] = "exten => s,n,Verbose(4, Step ".++$i.": {$grp['group_type']} HuntList '{$grp['description']}')";
		            	$_play_rec = basename($grp['announcement_file'], '.' . end(explode('.', $grp['announcement_file'])));
		            	if( $_play_rec )  $items[] =  " exten => s,n,Playback('snd_{$_tenant}/{$_play_rec}')";
		            	
		            	switch( $grp['group_type'] ){
		            		
		            		case 'queue':                     
                          $items[] = "exten => s,n,Dial(Local/s@internal-{$_tenant}-queue-{$DATA},,tT)" ;
                          break; 
                        ...
                        case 'extension':
                          ...
                          $EXTENSIONS = array();
                          $PHONES = array();                          
                          $exten_res=mysql_query("SELECT name FROM t_sip_users WHERE id in (" . implode(',',$EXTENSIONS_IDS) .")");
                          while($r = mysql_fetch_assoc($exten_res)){
                          	  $EXTENSIONS[] = 'SIP/'.$r['name'];
                             $items[] =  "exten => s,n,Verbose(2,[ RingGroup $_rgname ADD Exten: SIP/{$r['name']}  ])";                          	  
                          }
                          ...
                          $APP = $RESULT_LIST ? "{$RESULT_LIST},{$grp['timeout']}" : "Local/s@general-{$_tenant}-error";
              ...
              $items[] = ';; if we are back we failed?';
		        ...
		        switch( $row['default_action'] ) {
                  ...
                  case "play_rec":
       	         ...
       	         case "voicemail":
          	         ...
          	         $items[] = " exten => default,n,Voicemail({$r['vmail_box']})" ;          	   
          	         break;   
          	     	   
		          }
		          $items[] =  " exten => default,n,Hangup()\n" ; 
		       
         echo "[internal-{$_tenant}-ringgroup-{$_rgid}]\n";		       
		   if (!count($items))
